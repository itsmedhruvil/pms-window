  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className={cn(
          'relative p-1.5 text-primary-500 hover:text-dark-500 active:bg-primary-100 rounded-lg transition-colors',
          isOpen && 'bg-primary-100 text-dark-500'
        )}
      ...
      {isOpen && (
        <div
          ref={dropdownRef}
          className="fixed lg:absolute top-14 lg:top-full right-2 lg:right-0 mt-0 lg:mt-1 w-[90vw] max-w-[420px] max-h-[70vh] bg-white rounded-lg shadow-2xl border border-primary-200 z-50 flex flex-col animate-in fade-in slide-in-from-top-2 duration-150"
        >
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-3 border-b border-primary-200 flex-shrink-0">
            ...
            <div className="flex items-center gap-1">
              {notifications.length > 0 && (
                <>
                  ...
                  <button
                    type="button"
                    onClick={clearAll}
                    className="text-[9px] font-mono text-primary-400 hover:text-red-600 px-1.5 py-1 flex items-center gap-1"
              ...
              <button
                ...
                className="text-primary-400 hover:text-dark-400 p-1"
              >
          ...
          <div className="flex-1 overflow-y-auto">
            {notifications.length === 0 ? (
              <div className="px-4 py-8 text-center">
                ...
                <p className="text-[10px] font-mono text-primary-400">No notifications</p>
              </div>
            ) : (
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        isUnread ? 'bg-blue-50/50 hover:bg-blue-50' : 'hover:bg-primary-50'
                      )}
                    >
                      {/* Unread indicator */}
                      {isUnread && (
                      ...
                      <div className="flex-1 min-w-0">
                        <p className={cn(
                          ...
                          isUnread ? 'font-semibold text-dark-500' : 'font-medium text-dark-600'
                        )}>
                          {notification.title}
                        </p>
                        <p className="text-[10px] text-primary-500 mt-0.5 line-clamp-2">
                          {notification.body}
                        </p>
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-[8px] font-mono text-primary-400">
                      ...
                      <div className="flex items-center gap-0.5 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                        {isUnread && (
                          <button
                            type="button"
                            onClick={() => markAsRead(notification.id)}
                            className="p-1 text-primary-400 hover:text-blue-600"
                            title="Mark as read"
                          >
                            <CheckCheck className="w-3 h-3" />
                          </button>
                        ...
                        <button
                          ...
                          className="p-1 text-primary-400 hover:text-red-600"
                          title="Dismiss"
                        >
                          <X className="w-3 h-3" />
                        </button>
          ...
          {/* Footer */}
          {notifications.length > 0 && (
            <div className="px-4 py-2 border-t border-primary-200 bg-primary-50/50 flex-shrink-0">
              <button
                type="button"
                onClick={() => { clearAll(); setIsOpen(false); }}
                className="text-[9px] font-mono text-primary-400 hover:text-red-600 w-full text-center"
              >
